...
void msg_cb(const std_msgs::String::ConstPtr &msg)
{
    if (strcmp(msg->data.c_str(), "go") == 0 && waypoint_pose.empty() && target_pose_activate)
    {
        ...
        while (1)
        {
            ...
            double temp_dist = sqrt((target_pose.pose.position.x - x) * (target_pose.pose.position.x - x) + (target_pose.pose.position.y - y) * (target_pose.pose.position.y - y));
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber target_sub = nh.subscribe<geometry_msgs::PoseStamped>("gm_phd/target_pose", 10, target_cb);
    ros::Subscriber msg_sub = nh.subscribe<std_msgs::String>("gm_phd/message", 10, msg_cb);
    ros::Subscriber cur_pose_sub = nh.subscribe<geometry_msgs::PoseStamped>("mavros/local_position/pose", 10, cur_pose_cb);
